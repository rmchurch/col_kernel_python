...
l_ii = 24.0 - np.log(
    np.sqrt(sml["sml_den"] * 1e-6) / sml["sml_t_ev"]
)  # TODO: update with actual l_ii, this is l_ie with ion
...
for isp in range(f0_f.shape[1]):
    for inode in range(f0_nnode):
        energy_para = 0.5 * (vpara - sml["sml_flow"]) ** 2.0 / f0["f0_velo_aniso"]
        energy_perp = 0.5 * vperp**2.0
        energy = np.einsum(
            "i,j->ij", energy_perp, np.ones(energy_para.shape)
        ) + np.einsum("i,j->ij", np.ones(energy_perp.shape), energy_para)
        f0_f[inode, isp, :, :] = (
            vperp[:, np.newaxis]
            * sml["sml_den"]
            / (np.sqrt(f0["f0_velo_aniso"]) * sml["sml_t_ev"])
            * np.exp(-energy)
        )
if use_jet:
    ...
    datapath = (
        "/scratch/gpfs/marcoam/ml_collisions/data/xgc1/ti272_JET_heat_load/00094/"
    )
    ind = 50000
    phiind = 0
...
conv_factor = np.array(
    [
        1.0
        / np.sqrt(
            np.array(sml["sml_t_ev"])
            * (2 * np.pi * sml["sml_e_charge"] / np.array(sml["sml_mass"])) ** 3
        )
    ]
)
vperp1 = vperp.copy()
vperp1[0] = vperp1[1] / 3.0
...
m = np.array(sml["sml_mass"])
q = np.array(sml["sml_charge"])
vth = np.array([np.sqrt(np.array(sml["sml_t_ev"]) * sml["sml_ev2j"] / m)])
vol = 2 * np.pi * f0_dsmu * f0_dvp * np.einsum("i,j->ij", vth**3.0, vperp)
vol[:, 0] = 0.5 / 3.0 * vol[:, 1]  # would the 1/3 be cancelled by conv_factor?
vol[:, -1] = 0.5 * (vperp.size - 1.0 / 3.0) / (vperp.size - 1.0) * vol[:, -2]

# convert f0_f to the collision operator representation
if use_ion:
    fhat = f0_f.copy()
else:
    fhat = np.einsum("ijkl,j,k->ijkl", f0_f, conv_factor, 1.0 / vperp1)
...
def calcMoments(f):
    ...
    prefac = np.array(
        [0.5 * np.array(sml["sml_t_ev"])]
    )  # this is the 1/2*m*vth^2 factor, but normalizes out to this
    Tperp = np.einsum("i,j,kijl,ij->ik", prefac, vperp1**2.0, f, vol) / den
    Tpara = 2.0 * np.einsum("i,l,kijl,ij->ik", prefac, vpara**2.0, f, vol) / den
...
for i in range(sml["sml_nstep"]):
    ...
    den[..., i], upar[..., i], Tperp[..., i], Tpara[..., i], entropy[..., i] = (
        calcMoments(fhat)
    )
    print(
        "n: %.2e \t upar: %.2e \t Tperp: %.2e \t Tpara: %.2e \t Nneg: %d "
        % (den[0, 0, i], upar[0, 0, i], Tperp[0, 0, i], Tpara[0, 0, i], Nneg)
    )

    temp = (Tpara[..., i] + 2 * Tperp[..., i]) / 3.0
    print(temp)
    fin = F.pad(torch.from_numpy(fhat), (0, 1, 0, 0), mode="replicate")
    fdf = (
        collisions.forward(fin.cuda(), torch.from_numpy(temp).cuda())
        .detach()
        .cpu()
        .numpy()
    )  # pre- and post-processing in forward() pass
